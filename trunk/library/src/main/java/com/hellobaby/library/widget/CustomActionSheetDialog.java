...
public class CustomActionSheetDialog extends BottomBaseDialog<CustomActionSheetDialog> {
     ...
     * ListView Item颜色
     */
    private int mLvItemBgColor = Color.parseColor("#ddffffff");
    /**
     ...
     */
    private ArrayList<MyDialogMenuItem> mContents = new ArrayList<>();
    //教师model
    private ArrayList<TeacherModel> mTeacherModels = new ArrayList<>();
    ...
    public CustomActionSheetDialog(Context context, ArrayList<MyDialogMenuItem> baseItems, View animateView) {
    ...
    public CustomActionSheetDialog(Context context, String[] items, View animateView) {
        ...
        for (String  item : items) {
            MyDialogMenuItem customBaseItem = new MyDialogMenuItem(item, 0,R.color.white);
            mContents.add(customBaseItem);
        }
        init();
    }
    public CustomActionSheetDialog(Context context, String[] items) {
        super(context);
        mContents = new ArrayList<>();
        for (String  item : items) {
            MyDialogMenuItem customBaseItem = new MyDialogMenuItem(item, 0,R.color.white);
    ...
    public CustomActionSheetDialog(Context context, View animateView,List<TeacherModel> mTeacherModels) {
        super(context, animateView);
        mContents = new ArrayList<>();
        for (TeacherModel  item : mTeacherModels) {
            MyDialogMenuItem customBaseItem = new MyDialogMenuItem(StringUtils.getTeacherNamePosition(item.getTeacherName(),item.getPosition()), 0,R.color.white);
            mContents.add(customBaseItem);
        }
        init();
    }
    ...
    public CustomActionSheetDialog lvItembgColor(int lvItembgColor){
        this.mLvItemBgColor = lvItembgColor;
        return this;
    }
    ...
    class ListDialogAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            final MyDialogMenuItem item = mContents.get(position);
            ...
            tvItem.setBackgroundResource(item.colorId);
            llItem.addView(tvItem);
            llItem.setBackgroundColor(mLvItemBgColor);
            float radius = dp2px(mCornerRadius);
            if (mIsTitleShow) {
