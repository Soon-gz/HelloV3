...
public class DateUtil {
    ...
    public static String getDescriptionTimeFromTimestampInSchool(long timestamp) {
        long currentTime = System.currentTimeMillis();
        long timeGap = (currentTime - timestamp) / 1000;// 与现在时间相差秒数
        String timeStr = null;
        if (timeGap > DAY) {// 1天以上
            timeStr = getFormatTimeFromTimestamp(timestamp,"yyyy年MM月dd日");
        } else if (timeGap > HOUR) {// 1小时-24小时
            timeStr = timeGap / HOUR + "小时前";
        } else if (timeGap > MINUTE) {// 1分钟-59分钟
            timeStr = timeGap / MINUTE + "分钟前";
...
//        return Integer.parseInt(String.valueOf(between_days));
     ...
     * @return
     */
    public static String long2ClassAssistantTime(long time)
    {
        Date date=new Date(time);
     ...
     * @return
     */
    public static String getAttendTime(String time)
    {
        SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd");
        Date date = null;
        try {
...
//        return Integer.parseInt(String.valueOf(between_days));
    ...
    /**
     * 获取未来一周的日期，星期一的日期 - 星期天的日期
     * @return
     */
    public static List<String> getWeekList(final int count) {
        ...
        for (int i = 0; i < count; i++) {
            c.setTime(tm);
            c.add(Calendar.DAY_OF_WEEK, 7*i);
            //周一
            monday.setTime(c.getTime());
            monday.set(Calendar.DAY_OF_WEEK, monday.getFirstDayOfWeek());
            monday.add(Calendar.DATE, 1);
            //周日
            sunday.setTime(c.getTime());
            sunday.set(Calendar.DAY_OF_WEEK, monday.getFirstDayOfWeek() + 6);
            sunday.add(Calendar.DATE, 1);
            String week = sdf.format(monday.getTime())+"-"+sdf.format(sunday.getTime());
            resultList.add(week);
        }
        return resultList;
    }
     ...
     * @date 2017年11月23日
     */
    public static List<Date> getDateFromTwoDate(Date dateFirst, Date dateSecond) {
        List<Date> dates=new ArrayList<>();

        if (getFormatTimeFromTimestamp(dateFirst.getTime(),"yyyy.MM.dd").equals(getFormatTimeFromTimestamp(dateSecond.getTime(),"yyyy.MM.dd"))){
            if (dateFirst.getTime() < System.currentTimeMillis()){
                dates.add(dateFirst);
            }
            return dates;
        }
