...
public class VideoRecordSurface extends SurfaceView implements SurfaceHolder.Callback, MediaRecorder.OnErrorListener {
    ...
    private int cameraPosition = 0;//切换前置后置摄像头 0 前置 1 后置
    ...
    public void record(final OnRecordListener onRecordListener,int orientationHintDegrees) {
        ...
        createRecordDir();
        try {
            initRecord(orientationHintDegrees,cameraPosition);
            // 时间计数器重新赋值
            mTimeCount = 1;
            mTimer = new Timer();
     ...
     * @date 2016-06-21
     */
    private void initRecord(int orientationHintDegrees,int cameraPosition) {
        try {
            ...
            if (cameraPosition == 0){
                ...
                int newOrientaion = orientationHintDegrees+180;
                mMediaRecorder.setOrientationHint(newOrientaion);// 前置输出旋转270度，保持竖屏录制
            }
            //3.设置输出
