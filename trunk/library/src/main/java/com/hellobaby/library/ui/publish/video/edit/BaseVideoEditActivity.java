package com.hellobaby.library.ui.publish.video.edit;
 ...
 * ================================================
 */
public abstract class BaseVideoEditActivity extends AppCompatActivity {
    private static final String TAG = BaseVideoEditActivity.class.getSimpleName();
    ...
    private static final long MAX_CUT_TIME = 10000;//最大剪辑时间
    ...
    private String path;
    private String path_cut;
    private File sourceFile;
    private File cutputFile;
    private long leftProgress, rightProgress;
    private long scrollPos = 0;
    private int mScaledTouchSlop;
    private int lastScrollX;
    private boolean isSeeking;
    private ImageView eft_arrow_edit;
    private Button video_edit_complete;
    private boolean isMoved = false;

    public abstract void videoEditSuccess(String videoPath);
    ...
    private void initData() {
        if (Build.VERSION.SDK_INT >= 21) {
            getWindow().setStatusBarColor(getResources().getColor(com.hellobaby.library.R.color.sbc_snippet_text));
            getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN|View.SYSTEM_UI_FLAG_LAYOUT_STABLE);
        }
        path = getIntent().getStringExtra("video_mp4");
        path_cut = Const.videoPath+File.separator+"video_cut_"+System.currentTimeMillis()+".mp4";
        //for video check
    ...
    private void initView() {
        ...
        eft_arrow_edit = (ImageView) findViewById(R.id.eft_arrow_edit);
        video_edit_complete = (Button) findViewById(R.id.video_edit_complete);
        eft_arrow_edit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                finish();
            }
        });
        video_edit_complete.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                int startTime = (int) (leftProgress/1000);
                int endTime = (int) (rightProgress/1000);
                TrimVideoUtils.getInstance().startTrim(true,startTime,endTime,sourceFile,cutputFile);
                videoEditSuccess(cutputFile.getAbsolutePath());
            }
        });
    }
    ...
    private void initEditVideo() {
        ...
        if (isOver_60_s) {
            ...
            seekBar.setSelectedMaxValue(MAX_CUT_TIME);
        } else {
...
//        Log.d(TAG, "-------OutPutFileDirPath--->>>>" + OutPutFileDirPath);
...
//            Log.d(TAG, "-------scrollX:>>>>>" + scrollX);
            ...
            if (scrollX == -UIUtil.dip2px(BaseVideoEditActivity.this, 35)) {
                scrollPos = 0;
            } else {
                ...
                isSeeking = true;
                scrollPos = (long) (averageMsPx * (UIUtil.dip2px(BaseVideoEditActivity.this, 35) + scrollX));
...
//        Log.d(TAG, "--anim--onProgressUpdate---->>>>>>>" + mVideoView.getCurrentPosition());
    ...
    private static class MainHandler extends Handler {
        private final WeakReference<BaseVideoEditActivity> mActivity;

        MainHandler(BaseVideoEditActivity activity) {
            mActivity = new WeakReference<>(activity);
        }

        @Override
        public void handleMessage(Message msg) {
            BaseVideoEditActivity activity = mActivity.get();
            if (activity != null) {
    ...
    private final RangeSeekBar.OnRangeSeekBarChangeListener mOnRangeSeekBarChangeListener = new RangeSeekBar.OnRangeSeekBarChangeListener() {
        @Override
        public void onRangeSeekBarValuesChanged(RangeSeekBar bar, long minValue, long maxValue, int action, boolean isMin,boolean isMax, RangeSeekBar.Thumb pressedThumb) {
