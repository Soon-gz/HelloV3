...
public class CommAlterActivity extends BaseLibTitleActivity {
    ...
    protected void initViewsAndEvents(@Nullable Bundle savedInstanceState) {
        ...
        if(mCommAlterBean.getInputLength()>0){
            et.setFilters(
            new InputFilter[] { new InputFilter.LengthFilter(mCommAlterBean.getInputLength()) }
            );
        ...
        et.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                if (newValue.isEmpty() || mCommAlterBean.getOldValue().equals(newValue)) {
                    ...
                    if (mCommAlterBean.isInputTypeEmail() && !StringUtils.isEmail(newValue)) {
                        //邮箱类型
                        bIvRight.setVisibility(View.GONE);
                        return;
                    }
