package com.abings.baby.teacher.jpush;
...
public class MyReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
        ...
        if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
        	...
        	LogZS.i("[MyReceiver] 接收到推送下来的自定义消息携带参数: " + bundle.getString(JPushInterface.EXTRA_EXTRA));
            ...
            LogZS.i("[MyReceiver] 接收到推送下来的通知");
            int notifactionId = bundle.getInt(JPushInterface.EXTRA_NOTIFICATION_ID);
            LogZS.i("[MyReceiver] 接收到推送下来的通知的ID: " + notifactionId);
        	
        } else if (JPushInterface.ACTION_NOTIFICATION_OPENED.equals(intent.getAction())) {
            LogZS.i("[MyReceiver] 用户点击打开了通知");
        	...
        	LogZS.i("[MyReceiver]" + intent.getAction() +" connected state change to "+connected);
        } else {
		...
		for (String key : bundle.keySet()) {
			if (key.equals(JPushInterface.EXTRA_NOTIFICATION_ID)) {
				...
				try {
					...
					LogZS.i("Get message extra JSON error!");
				}
