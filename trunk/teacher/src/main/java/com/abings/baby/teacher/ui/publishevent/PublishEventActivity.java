...
public class PublishEventActivity extends BaseTitleActivity implements PublishEventMvpView {
    ...
    EditText etPcount;
    @BindView(R.id.event_et_childcount)
    EditText etCcount;
    ...
    protected void btnRightOnClick(View v) {
        ...
        eventModel.setEventBaby(Integer.valueOf(etCcount.getText().toString().trim()));
        try {
        ...
        if(Integer.valueOf(etCcount.getText().toString().trim())==0){
            ToastUtils.showNormalToast(bContext,"要有宝宝人数");
            return;
        }
