...
public class RePwdPresenter extends BasePresenter<RePwdMvpView> {
     ...
     * @param phone
     */
    public void teacherCheckPhoneExists(String phone) {
        resetSubscription();
        mDataManager.teacherCheckPhoneExists(phone)
     ...
     * @param smsCode
     */
    public void teacherChangeTeacherPassword(String phone, String pwd, String smsCode) {
        resetSubscription();
        bMvpView.showProgress(true);
        mDataManager.teacherChangeTeacherPassword(phone, MD5Utils.encodeMD5(pwd), smsCode)
                .compose(RxThread.<BaseModel<String>>subscribe_Io_Observe_On())
                .subscribe(new SubscriberClass<String>(bMvpView) {
                    @Override
                    protected void callSuccess(String s) {
                        bMvpView.showMsg("修改密码成功");
