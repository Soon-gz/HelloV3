                ...
                public void onClick(View v) {
                    BottomDialogUtils.getBottomListDialog(bContext, items, new BottomDialogUtils.OnItemClickListener() {
                        ...
                        public void onItemClick(AdapterView<?> parent, View view, int position, String item, long id) {
                            ...
                            if(position==1) {
                                if(null!=schoolItem)
                                ShareBottomDialog.getShareUrlBottomDialog(bContext, null,schoolItem.getName(),"来自哈喽宝贝家长端",new UMImage(bContext,schoolItem.getListUrl().get(0)),schoolItem.getNewsUrl());
                                else
                                    ShareBottomDialog.getShareUrlBottomDialog(bContext, null,informationModel.getTitle(),"来自哈喽宝贝家长端",new UMImage(bContext,informationModel.getNewInfoImageurls().split(",")[0]),informationModel.getDetailsUrl());
                            }
                        }
                    });
                }
